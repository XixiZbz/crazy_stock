...
for data in datas:
    ...
    update_date = {key:value for key,value in data.items() if key not in ['序号','代码']}
    ...
    data['update_date'] = str(arrow.now().date())
    update_date["update_date"] = str(arrow.now().format("YYYY-MM-DD HH:mm:ss"))
    if is_exit:
